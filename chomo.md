...
## 9-1단계 — imuPreintegration 추가 (회전 오차 개선) ✅
**배경**: lio_sam_go2가 회전 오차가 좀 있어, IMU를 팩터그래프에 넣는 `imuPreintegration` 노드를 추가.
- `liosam_go2_test.launch.py`에 `lio_sam_go2_imuPreintegration` 노드 추가.
- **충돌/에러 없음 확인**: mapOptimization은 `odom→velodyne_base`, imuPreintegration은 `odom→base_link` 발행 → child 프레임 달라 충돌 없음. `lidarFrame == baselinkFrame`(둘 다 base_link)이라 imuPreintegration의 lidar↔base TF lookup을 건너뜀 → 예전 "extrapolation" 에러도 안 남.
- **fork 버그 수정** (`imuPreintegration.cpp` ~143줄): TF의 부모 frame_id/stamp를 채우는 코드가 `if(lidarFrame != baselinkFrame)` 블록 안에만 있어, 두 프레임이 같을 때 frame_id가 비어 `TF_NO_FRAME_ID`로 `odom→base_link`가 버려졌음. → convert 후 `ts.header.frame_id=odometryFrame; ts.header.stamp=odomMsg->header.stamp;`를 명시 추가.
- **검증**: rebuild 후 TF_NO_FRAME_ID 0개, `odom→base_link` TF 정상 해석.

## 9-2단계 — 맵 자동저장 (PCD) ✅
**요청**: 맵핑한 결과를 자동으로 PCD 저장.
- LIO-SAM에 내장 서비스 **`/lio_sam/save_map`** (`lio_sam_go2/srv/SaveMap`: `resolution`, `destination`) 존재. 저장물: `GlobalMap.pcd`(전체 맵), Corner/Surf/trajectory/transformations.pcd.
  - ⚠️ 저장 코드가 `rm -r $HOME/<destination>` 후 생성 → **전용 폴더만** 쓸 것.
- **주기적 자동저장 노드 추가**: `go2_traversability/scripts/auto_save_map.py` (rclpy, 타이머마다 save_map 호출). Ctrl+C orphan 문제가 있어 "종료 시 저장" 대신 **주기 저장**(kill -9에도 마지막 저장본 보존).
- `liosam_go2_test.launch.py`에 통합 + 인자: `auto_save`(기본 true), `save_interval`(기본 30.0s), `save_dir`(기본 `/go2_maps/`).
- **검증**: 15초 주기로 `~/go2_maps/GlobalMap.pcd` 등 생성, `auto-save #1: success=True` 확인.

### ▶ 저장 위치 / 사용
- 기본 저장 위치: **`~/go2_maps/`** (`$HOME` + `save_dir`). `GlobalMap.pcd`가 전체 맵.
- 자동저장 끄기: `ros2 launch ... liosam_go2_test.launch.py auto_save:=false`
- 주기·폴더 변경: `save_interval:=60.0 save_dir:=/my_maps/`
- 수동 1회 저장: `ros2 service call /lio_sam/save_map lio_sam_go2/srv/SaveMap "{resolution: 0.0, destination: '/go2_maps/'}"`
- 확인: `pcl_viewer ~/go2_maps/GlobalMap.pcd`

## 10단계 — 3D PCD → 2D occupancy grid (Nav2용) 진행중
**목표**: LIO-SAM `GlobalMap.pcd` → 2D 맵(.pgm/.yaml) → Nav2 경로 생성.
- 변환 스크립트 신규: `go2_traversability/scripts/pcd_to_2dmap.py` (PCD 바이너리 직접 파싱 → 높이 띠로 장애물 추출 → occupancy grid → PGM+YAML).
- 입력: `~/go2_maps/GlobalMap.pcd` (459,378 pts). 결과: 그리드 1067×1462 @0.05m.
  - `~/go2_maps/map.{pgm,yaml}` — 높이 0.15~2.0m (점유 52k, 노이즈 있음)
  - `~/go2_maps/map_clean.{pgm,yaml}` — 높이 0.3~1.2m 슬라이스 + 셀당 3점+ (점유 9k, **Nav2 권장**)
- 사용법: `python3 .../pcd_to_2dmap.py ~/go2_maps/GlobalMap.pcd ~/go2_maps/map_clean --resolution 0.05 --min-h 0.3 --max-h 1.2 --min-points 3`
- 맵 확인: PGM→PNG 후 이미지 뷰. 건물 내부(벽·방·복도) 형상 확인됨.

### 다음 (Nav2)
- Nav2 미설치 → **사용자 sudo apt 필요**: `sudo apt install -y ros-humble-navigation2 ros-humble-nav2-bringup ros-humble-pointcloud-to-laserscan`
- 정적맵 내비 구조: map_server(map_clean) + **AMCL 측위**(pointcloud_to_laserscan로 /ouster/points→/scan) + LIO-SAM odom(odom→base_link) + planner/controller.
- AMCL이 map→odom을 소유하므로, Nav2 연동 시 LIO-SAM의 static map→odom은 제거 필요.

## 11단계 — Nav2 정적맵 내비 (AMCL) 동작 ✅
- 신규 통합 launch: `go2_traversability/launch/nav2_liosam.launch.py`
  - Ouster + LIO-SAM odom(4노드, static map→odom 없음) + base_link→os_sensor static TF
  - `pointcloud_to_laserscan`: /ouster/points → /scan (target os_sensor, 높이 -0.5~0.5)
  - nav2_bringup `bringup_launch.py`(map_server+AMCL+planner+controller+bt+lifecycle) + RViz
- Nav2 파라미터: `config/nav2_params.yaml` (기본 복사 후 `robot_radius 0.30`, `base_footprint`→`base_link` 전부 변경, map=`~/go2_maps/map_clean.yaml`).
- **핵심 버그 수정 (타임스탬프)**: Ouster `timestamp_mode: TIME_FROM_INTERNAL_OSC`(센서클럭 ~3195s)라 LIO-SAM TF가 센서시간으로 스탬프 → Nav2(ROS시간 ~1.78e9s)와 불일치로 모든 TF 조회 실패("extrapolation"). → `driver_params.yaml`을 **`TIME_FROM_ROS_TIME`**으로 변경(소스+설치본). 이후 모든 클럭 일치.
- **검증**: 초기위치 발행 후 `/tf`에 `map→odom`(AMCL) + `odom→base_link`(LIO-SAM) 모두 발행, 전체 체인 `map→odom→base_link→os_sensor` 연결. nav2 localization/navigation lifecycle 모두 active, /scan 8Hz.

### ▶ 실행 / 사용 (RViz)
```bash
ros2 launch go2_traversability nav2_liosam.launch.py
# 다른 맵: ... nav2_liosam.launch.py map:=$HOME/go2_maps/map.yaml
```
1. RViz에서 **"2D Pose Estimate"** → 맵에서 로봇 실제 위치+방향 클릭·드래그 (AMCL 초기 측위).
2. **"Nav2 Goal"** → 목적지 클릭·드래그 → **경로 생성** + 주행(cmd_vel).
