    ...
    def __init__(self, param: Parameter):
        ...
        if param.use_chainer:
            ...
            try:
                ...
                # No torch (e.g. JetPack CUDA torch wheel unavailable). Fall back to a
                # passthrough so the 2.5D elevation map still runs. Install torch later
                # to automatically restore the learned traversability filter.
                print(
                    "[elevation_mapping_cupy] WARN: torch traversability filter "
                    f"unavailable ({e}); using passthrough (traversability=1.0)."
                )
                self.traversability_filter = get_filter_passthrough(
