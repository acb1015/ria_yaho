...
class PassthroughTraversabilityFilter:
    """cupy-only stand-in for the learned (torch/chainer) traversability filter.

    Used when neither torch nor chainer is available, so the 2.5D elevation map
    can still run. It returns full traversability (1.0) for every valid cell and
    matches the NN filter's output shape ``(1, 1, H-6, W-6)`` and value range
    (the real filter outputs ``exp(-x)`` in ``(0, 1]``). Re-enable the real
    filter simply by installing torch -- no code change needed.
    """

    def __call__(self, elevation_cupy):
        elevation_cupy = elevation_cupy.astype(cp.float32, copy=False)
        h = elevation_cupy.shape[0] - 6
        w = elevation_cupy.shape[1] - 6
        return cp.ones((1, 1, h, w), dtype=cp.float32)


def get_filter_passthrough(*args, **kwargs):
    return PassthroughTraversabilityFilter()
