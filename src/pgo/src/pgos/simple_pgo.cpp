...
{
    // NaN/Inf guard: drop invalid odometry (e.g. when lidar stream drops out)
    // so corrupted poses never reach the pose graph or the loop-search kdtree.
    if (!cloud_with_pose.pose.t.allFinite() || !cloud_with_pose.pose.r.allFinite())
        return false;
