...
public:
    ...
    void imuOdometryHandler(const nav_msgs::msg::Odometry::SharedPtr odomMsg)
    {
        ...
        tf2::convert(tCur, ts);
        // frame_id/stamp are only set inside the (lidarFrame != baselinkFrame)
        // branch above; set them explicitly so odom->base_link is valid when
        // lidarFrame == baselinkFrame too (otherwise TF_NO_FRAME_ID, dropped).
